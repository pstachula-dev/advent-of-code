  ...
  solution: (input) => {
    ...
    const checkCards = (cards: string[]) => {
      ...
      for (const [, val] of map.entries()) {
        ...
        if ((val === 3 && pair === 2) || (val === 2 && pair === 3)) {
          return Type.Full;
        }

        if (val === 2 && pair === 2) return Type.TwoPair;
        if (val === 5) return Type.Five;
        if (val === 4) return Type.Four;
      }

      if (pair === 2) return Type.OnePair;
      if (pair === 3) return Type.Three;
    };
